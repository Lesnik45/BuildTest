...
void InterfaceDoExit()
{
	...
	switch(interfaceResultCommand)
	{
		...
		case RC_INTERFACE_DO_BETA_TELEPORT:
		     LaunchBetaTeleportScreen();
        break;
		// boal <--
		...
		case RC_INTERFACE_LOCKPICKING:
			LaunchLockpicking();
		break;
		
		case RC_INTERFACE_LOCKPICKED:
			aref ar;
			makearef(ar, pchar.lockpicking);
			LaunchItemsBox(&ar);
		break;
		
		case RC_INTERFACE_LOCKPICKED_LOCATION:
		    if(CheckAttribute(&locations[FindLocation(pchar.location)], "" + pchar.boxname + ".key")) DeleteAttribute(&locations[FindLocation(pchar.location)], "" + pchar.boxname + ".key");
		    if(CheckAttribute(&locations[FindLocation(pchar.location)], "" + pchar.boxname + ".Closed")) DeleteAttribute(&locations[FindLocation(pchar.location)], "" + pchar.boxname + ".Closed");
			string sBoxname = pchar.boxname;
			Log_TestInfo(""+ sBoxname);
		    locations[FindLocation(pchar.location)].(sBoxname).opened = true;
	...
	switch(ControlName)
	{
        case "TimeScale":
			...
			if(IsPerkIntoList("TimeSpeed"))
			{
				SetTimeScale(1.0);
				TimeScaleCounter = 0;
                Log_SetStringToLog("Ускорение времени x1");
				DelPerkFromActiveList("TimeSpeed");
			}
			else
			{
				...
				TimeScaleCounter = 4;
                Log_SetStringToLog("Ускорение времени x2");
				AddPerkToActiveList("TimeSpeed");
			}
		break;
		...
		case "UsePotionv1":
            ...
            {	
				pchar.GenQuest.Potion_choice.variant = "v1";
				...
				if(LAi_IsPoison(pchar))
				{
					if (UseBestPotion(pchar, true) == 0) 
					{
						...
						{
							Log_SetStringToLog("Лечебное зелье отсутствует!");
						}
					}
				}
				else 
		...
		case "UsePotionv2":
			if (bLandInterfaceStart)
            {
				pchar.GenQuest.Potion_choice.variant = "v2";
        ...
        case "UsePoison":  // boal KEY_F
			if (bLandInterfaceStart)
            {
				if(CheckCharacterItem(pchar, pchar.GenQuest.Poison_choise))
				{
				    DoCharacterUsedPoison(pchar, pchar.GenQuest.Poison_choise);
				}
				else 
				{
				  	Log_SetStringToLog("Выбранная отрава отсутствует в инвентаре!");
				}	
	        }
	    break;
...
bool QuickSaveGameEnabledHardcore()
{
	...
	if (bHardcoreGame)
	{
	    ...
	    if( idxLoadLoc!=-1 )
	    {
	        ...
	        {
				// Rebbebion, теперь на хардкоре можно сохраняться и в тавернах тоже
	            if (Locations[idxLoadLoc].type == "church" || Locations[idxLoadLoc].type == "tavern")
	            {
	                TmpBool = true;
	            }
	        }
