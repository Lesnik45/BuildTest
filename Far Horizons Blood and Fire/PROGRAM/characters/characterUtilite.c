...
float GetSailRPD(ref _refCharacter) // процент ремонта парусов в день
{
	...
	IsCharacterEquippedTalisman(_refCharacter, "totem_21") repairSkill = repairSkill * 2;
...
float GetSailSPP(ref _refCharacter) // количество парусины на один процент починки
{
	...
	ret = ret * isEquippedTalismanUse(_refCharacter, "totem_11", 1.0, 0.85);
...
string FindCharacterItemByGroup(ref chref, string groupID)
{
	...
	for(i=TOTAL_ITEMS-1; i>=0; i--)
	{
		...
		if(groupID==GUN_ITEM_TYPE)
		{
			...
			if(n < 4)
			{
				if( IsCharacterPerkOn(chref,"Musketeer") ) {return refItm.id;}
				else continue;
			}
            //boal -->
            if( IsCharacterPerkOn(chref,"Shooter") )	{return refItm.id;}
            // boal <--
			continue;
		}
...
bool IsEquipCharacterByTalisman(ref chref, string itemID)
{
	aref 	arEquip;
	string 	sAttr;
	int 	i, q;
...
int isEquippedTalismanUse(ref rChar, string sItem, float fOff, float fOn)
{
	int returnvalue = 0;
	if(IsCharacterEquippedTalisman(rChar, sItem)) returnvalue = fOn;
	else returnvalue = fOff;
	
	return returnvalue;
}

bool IsCharacterEquippedTalisman(ref rChar, string itemID)
{
	int 	iOfficer = -1;
	ref 	rItem, sld;
	string  sKind = "";
	...
	{
		if(IsEquipCharacterByTalisman(pchar, itemID)) return true;
		...
		if(CheckAttribute(rItem, "kind")) 
		{
			...
			if(sKind == "fighter")
			{
				...
				{		
					...
					if(iOfficer != -1)
					{
						if(IsEquipCharacterByTalisman(&characters[iOfficer], itemID)) return true;						
					}
				}	
			}
			else
			{
				...
				if(iOfficer != -1)
				{
					return IsEquipCharacterByTalisman(&characters[iOfficer], itemID);
				}
			}	
			return false;
		}	
		else
		{
			for (int io = 0; io < GetNotQuestPassengersQuantity(rChar); io++)
			{
				...
				if(iOfficer != -1)
				{
					...
					{
						if(IsEquipCharacterByTalisman(sld, itemID)) return true;
					}					
				}
			}
			return false;
	...
	return IsEquipCharacterByTalisman(rChar, itemID);
}
...
bool CheckCharacterTalent(ref chref, string talentName)
{
	if( CheckAttribute(chref,"talents.list."+talentName) ) return true;
	return false;
}

bool SetCharacterTalent(ref chref, string TalentName)
{
	chref.talents.list.(TalentName) = true;
	switch (TalentName)
	{
		case "Agility": //Ловкость CheckCharacterTalent(pchar, "Agility")
		break;
		case "Seducer": //Обольститель
		break;
		case "KeenEye": //Зоркий глаз
		break;
		case "ShadowMaster": //Мастер теней CheckCharacterTalent(pchar, "ShadowMaster")
		break;
		case "BigBoy": //Крепыш CheckCharacterTalent(pchar, "BigBoy")
		break;
		case "Thug": //Головорез CheckCharacterTalent(pchar, "Thug")
		break;
		case "NaturalLeader": //Прирожденный лидер CheckCharacterTalent(pchar, "NaturalLeader")
		break;
		case "DiligentStudent": //Прилежный ученик CheckCharacterTalent(pchar, "DiligentStudent")
		break;
	}
