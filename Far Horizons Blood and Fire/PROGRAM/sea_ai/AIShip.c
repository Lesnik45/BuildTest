...
void Ship_CheckFlagEnemy(ref rCharacter)
{
    ...
    {  // смотрим флаг у ГГ и узнаем в нем...
		...
		int iStells1, iStells2;
		iStells1 = rand(100) + rand(20) + rand(rep);
		iStells2 = fSneak * 10 * iClass * (6-iCompan);
		if(sti(rCharacter.Ship.Type) == SHIP_FORT)
		{
			if(CheckCharacterPerk(pchar, "AGOS")) iStells1 = iStells1 * 0.5;
		}
		else
		{
			if(CheckCharacterPerk(pchar, "AGOS")) iStells1 = iStells1 * 0.65;
		}
		
		iStells1 = iStells1 * IsCharacterEquippedTalisman(pchar, "totem_10", 1.0, 0.85);
		
		if (iStells1 > iStells2)
		{
...
{         // че-то распук метод "неподецки" - переделал 29.07.06 boal
	...
	fMultiply = fMultiply * isEquippedTalismanUse(rOurCharacter, "totem_08", 1.0, 0.9);
...
void Ship_ApplyHullHitpoints(ref rOurCharacter, float fHP, int iKillStatus, int iKillerCharacterIndex)
{
    ...
    if (iKillerCharacterIndex >= 0)
	{
		...
		if(IsCharacterEquippedTalisman(&Characters[iKillerCharacterIndex], "totem_24")) fPlus = fPlus * 1.15;
	}
	...
	if(IsCharacterEquippedTalisman(rOurCharacter, "totem_28")) fMinus = fMinus * 1.3;
...
void ShipDead(int iDeadCharacterIndex, int iKillStatus, int iKillerCharacterIndex)
{
	...
	{ 
		...
		FadeOutMusic(2);
		// убил ЛГ
		if (CheckCharacterPerk(rDead, "Shipwrecker"))
		{
	...
	{
        if (bDeadCompanion && GetRemovable(rDead)) // выживаем
        {
            //homo 22/06/07
	...
	if (bSeaLoaded && CheckAttribute(rCharacter, "SeaAI.Task.Target"))
	{
		if(sti(rCharacter.SeaAI.Task.Target) > 0)
		{
			...
			if (!bSkipTarget)
			{
				...
				// Черепашка (Нефритовая)
				if(IsCharacterEquippedTalisman(rTargetCharacter, "totem_03")) fRatio = fRatio * 0.9; 
...
void DropGoodsToSea()
{
	...
	for (int i=GOOD_MEDICAMENT; i<GOOD_CANNON_42; i++)
	{
		sGood = Goods[i].name;
