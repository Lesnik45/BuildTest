...
float Cannon_GetRechargeTime()
{
	...
	// фикс работы перков и фигурки Шиутекутли от EvgAnat, 23.10.23
	fMultiply *= AIShip_isPerksUse(aCharacter.TmpPerks.FastReload, 1.0, 0.9);
    fMultiply *= AIShip_isPerksUse(aCharacter.TmpPerks.ImmediateReload, 1.0, 0.5); 
    fMultiply *= isEquippedTalismanUse(aCharacter, "totem_12", 1.0, 0.9);
...
float Cannon_DamageEvent()
{
	...
	if(IsCharacterEquippedTalisman(aCharacter, "totem_30")) return 0.0;
