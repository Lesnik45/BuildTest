...
void LAi_CharacterAttack()
{
	...
	{
		int iRand = 0;
		if(CheckCharacterPerk(attack, "sliding")) = iRand = 20;
		if(CheckCharacterPerk(attack, "HardHitter")) = iRand = 30;
		if((rand(99) + 1) <= iRand)
		{
			isBlocked = false;
		}
...
void LAi_CharacterFire()
{
	...
	if(CheckAttribute(attack, "chr_ai.multidmg") && sti(attack.chr_ai.multidmg) > 0)
	{
		...
		for (int j = 0; j < num; j++)
		{
			...
			//{
                if(sBullet == "petard") LAi_Stunned_StunCharacter(enemy, 10, true);
				
                if(sBullet == "Poison_bomb") MakePoisonAttack(attack, enemy, 30, 100, 2, "Poison1");
