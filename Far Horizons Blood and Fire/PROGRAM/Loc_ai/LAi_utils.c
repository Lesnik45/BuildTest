...
ref LAi_CreateFantomCharacterEx(string model, string ani, string group, string locator)
{
	...
	// fix -->
    if (chr.model.animation == "towngirl" || chr.model.animation == "towngirl3")
    {
	...
	if(GetCharacterPos(chr, &x, &y, &z) == true && Dead_Char_num < (100))
	{
	    ...
	    else
	    {
            ...
            for(j = ItemDeadStartCount; j < ITEMS_QUANTITY; j++) // нужно оптимизировать!!! 137 - это много  начало с опред места
    		{
				...
				makeref(itm,Items[j]);
				if(CheckAttribute(itm, fname))
				{
					makearef(typeRef, itm.(fname));
					
                    //if (!CheckAttribute(typeRef, "rare") && MOD_BETTATESTMODE == "On") Log_Info("Error: Не найдет RARE для предмета = "+itm.id + " тип = " + fname);

                    float itemProb = 0.05;
	                if (CheckAttribute(itm, "rare"))
	                {
	                  	switch(itm.rare)
	                    {
                    ...
                    if (rand(1000) < (((itemProb + itemProb * nLuck / 20.0) / makefloat(MOD_SKILL_ENEMY_RATE)) * 930.0))
                    {
...
void MakePoisonAttack(aref attack, aref enemy, int duration, int chance, int damage, string sPoison)
{
	if(CheckCharacterPerk(enemy, "SnakeSkin") || IsCharacterEquippedTalisman(enemy, "totem_27"))
	{
		Log_TestInfo("У персонажа " +enemy.id+ " Черная роза/Змеиная кожа!");
		if(sPoison != "Poison7") return;
	}
	
	if(CheckAttribute(enemy, "PoisonResist"))
	{
		if(!CheckAttribute(enemy, "resistmessage"))
		{
		   	if(!IsMainCharacter(enemy)) Log_SetStringToLog("Персонаж " +GetFullName(enemy)+" неуязвим к отравлению.");
			enemy.resistmessage = true;
		}
		return;
	}
	
	int irand = (rand(99) + 1);
	Log_TestInfo("Процент - "+irand);
	Log_TestInfo("Яд - "+ chance);
	//Отравляем персонажа
	float poison = 0.0;
	...
	if(!CheckAttribute(enemy, "chr_ai.poison"))
	{
		if(irand <= chance)
		{
			if (enemy.index == GetMainCharacterIndex()) Log_SetStringToLog("Вы получили отравление от яда '"+ GetConvertStr("itmname_"+sPoison, "ItemsDescribe.txt") +"'!");
		    else Log_SetStringToLog("Персонаж " + GetFullName(enemy) + " получил отравление от яда '"+ GetConvertStr("itmname_"+sPoison, "ItemsDescribe.txt") +"'!");
			
	    	if(duration > 0)
    		{
	    	    enemy.chr_ai.poison = duration;
    		}
			else
			{
				enemy.chr_ai.poison = (rand(29) + 1);
			}
			
			if(damage > 0)
			{
				enemy.chr_ai.poisondmg = damage;
			}
			else
			{
				enemy.chr_ai.poisondmg = 2;
			}
		}
		
		if(sPoison == "Poison7")
		{
			enemy.chr_ai.NoAntidotes = true;
			enemy.chr_ai.NoAntidoteTime = 10;
			enemy.chr_ai.NoMixtureTime = 5;
			enemy.chr_ai.DrinkedAntidotes = 0;
			enemy.chr_ai.DrinkedMixtures = 0;
		}
	}
}
...
// mitrokosta - для запрета использования банок
bool LAi_CanUseBottle(aref chr)
{
	if (!CheckAttribute(chr, "DoNotUseBottle") && !LAi_IsBottleWork(chr) && MOD_SKILL_ENEMY_RATE > 2)
	{
		return true;
	}
	return false;
...
bool CharIsFromStockPirCity(ref rChar)
{
	...
	{
	    if(rChar.city == "Lavega" || rChar.City == "Lefransua" || rChar.City == "FortOrange") return true;
	    else return false;
	}
	else return false;
