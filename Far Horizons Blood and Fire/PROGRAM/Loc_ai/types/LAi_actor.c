...
void LAi_type_actor_Init(aref chr)
{
	...
	if (chr.model.animation == "mushketer" && !CheckAttribute(chr, "isMusketer.weapon") && chr.index != GetMainCharacterIndex())
	{	
...
void LAi_type_actor_CharacterUpdate(aref chr, float dltTime)
{
	...
	if (CheckAttribute(chr, "checkCharacters") && chr.chr_ai.type.state != "dialogwait") {
		float distance = 5.0;
		if (CheckAttribute(chr, "checkCharacters.distance")) {
			distance = stf(chr.checkCharacters.distance);
		}
		
		int num = FindNearCharacters(chr, distance, -1.0, -1.0, 0.01, true, true);
		...
		if(num > 0) {
			for(i = 0; i < num; i++) {
				...
				if (CheckAttribute(chr, "checkCharacters.reactionOnFightModeOn") && LAi_CheckFightMode(by)) {
					LAi_group_FightGroups(chr.chr_ai.group, by.chr_ai.group, true);
				}
				
				if (CheckAttribute(chr, "checkCharacters.dialog") && chr.checkCharacters.dialog.id == by.id) {
					// частота опроса, по умолчанию 10
					float interval = 10.0;
					if (CheckAttribute(chr, "checkCharacters.dialog.interval")) {
						interval = stf(chr.checkCharacters.dialog.interval);
					}
					
					float time = 0.0;
					if (CheckAttribute(chr, "checkCharacters.dialog.time")) {
						time = stf(chr.checkCharacters.dialog.time);
					}
					
					time -= dltTime;
					chr.checkCharacters.dialog.time = time;
					
					if (time < 0) {
						if (CheckAttribute(chr, "checkCharacters.dialog.node")) {
							chr.dialog.currentnode = chr.checkCharacters.dialog.node;
						}
						
						chr.chr_ai.type.lock = false; // разлочить таску
						LAi_ActorDialogNow(chr, by, "", -1);
						chr.checkCharacters.dialog.time = interval;
					}
				}
			}
		}
...
void LAi_type_actor_TemplateComplite(aref chr, string tmpl)
{
	...
	if(quest != "" && quest != "no") CompleteQuestName(quest, ""); // mitrokosta загрузка для сброса шаблона
}
