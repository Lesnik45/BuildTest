...
void LAi_type_patrol_Goto(aref chr)
{
	...
	{
		group = "PatrolElite";
		newloc = LAi_FindRandomLocator("PatrolElite");
	}
	// Rebbebion, стража генерал-губернатора Гаваны
	if (CheckAttribute(chr, "HGuardPatrol"))
	{
		group = "HGuardPatrol";
		newloc = LAi_FindRandomLocator("HGuardPatrol");
	}
	else
	{
	...
	{
		LAi_tmpl_goto_InitTemplate(chr);
		LAi_tmpl_goto_SetLocator(chr, group, newloc, -1.0);
...
void LAi_type_patrol_TestControl(aref chr)
{
	...
	else 
	{
		...
		if(CheckCharacterPerk(pchar, "AGOS")) iRand = iRand * 0.5;
		else
		{
			if(IsCharacterPerkOn(pchar, "Shadow")) iRand = iRand * 0.7;
		    else
	    	{
		   	 	if(IsCharacterPerkOn(pchar, "Spy")) iRand = iRand * 0.85;
	    	}
		}
	}
