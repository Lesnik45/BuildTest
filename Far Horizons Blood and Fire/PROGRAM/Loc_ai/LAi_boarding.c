...
void LAi_StartBoarding(int locType, ref echr, bool isMCAttack)
{
    ...
    if (CheckOfficersPerk(mchr, "MusketsShoot") && IsFort == false)
    {
		...
		ecrew = ecrew * isEquippedTalismanUse(mchr, "totem_08", 1.0, 0.9);
		ecrew = ecrew * isEquippedTalismanUse(echr, "totem_20", 1.0, 1.1);
	...
	if (CheckCharacterPerk(echr, "MusketsShoot") && IsFort == false)
    {
		...
		ecrew = ecrew * isEquippedTalismanUse(echr, "totem_08", 1.0, 0.9);
		ecrew = ecrew * isEquippedTalismanUse(mchr, "totem_20", 1.0, 1.1);
		...
		if (CheckOfficersPerk(mchr, "MusketsShoot"))
		{
			...
			for(i=1; i<=iQty; i++)
			{
				...
				{			
					...
					chr = GetCharacter(NPC_GenerateCharacter("GenChar_", model, "man", "mushketer", 5, sti(mchr.nation), 0, false, F_SAILOR));
		...
		if (!CheckAttribute(&Locations[locIndex], "CabinType"))
		{
			...
			for(i=1; i<=iQty; i++)
			{
				if (LAi_CheckLocatorFree("rld", sLocType+"mush"+i))
				{
					...
					chr = GetCharacter(NPC_GenerateCharacter("GenChar_", model, "man", "mushketer", 5, sti(boarding_enemy.nation), 0, false, F_SAILOR));
					chr.id = "GenChar_" + chr.index;
					chr.AboardFantom = true;
...
string LAi_GetBoardingMushketerModel(ref rCharacter)
{
	...
	if(iNation == PIRATE)
	{
		model = "mush_pir_" + (rand(5) + 1);
	}
	else
	{
		model = "mush_" + NationShortName(iNation) + "_" + (rand(4) + 1);
	}	
	return model;
}
