...
void BLI_ExecuteCommand()
{
	...
	case "BI_HackBox":
    	if(CheckCharacterPerk(pchar, "Safecracker"))
    	{
    		...
    		{
    			Log_SetStringToLog("Заперто! Жаль, нет отмычки...");
	...
	case "BI_ActivateRush":
		DoCharacterUsedItem(pchar, "berserker_potion");
	break;
	// boal
    case "BI_Exit_Deck":
...
void BLI_SetObjectData()
{
	...
	objLandInterface.Commands.HackBox.enable	    = true;
 	objLandInterface.Commands.HackBox.picNum		= 51;
	objLandInterface.Commands.HackBox.selPicNum		= 35;
	objLandInterface.Commands.HackBox.texNum		= 0;
	objLandInterface.Commands.HackBox.event			= "BI_HackBox";
	objLandInterface.Commands.HackBox.note			= LanguageConvertString(idLngFile, "land_HackBoxBox");
	...
	if(nLoc >= 0) {
		...
		if(nFile >= 0) {
			...
			if (locations[nLoc].id.label == "Tavern") // Rebbebion, вывод названия таверны в правом верхнем углу
			{
			    objLandInterface.textinfo.locationname.text = LanguageConvertString(nFile, "Tavern") + " '" + XI_ConvertString(""+locations[nLoc].fastreload+"TavernName") + "'";
			}
			else
			{
...
void BLI_SetPossibleCommands()
{
	...
	if(CheckCharacterItem(pchar, "berserker_potion") && !CheckAttribute(pchar, "BerserkerPotionPenalty"))
	{
		objLandInterface.Commands.ActivateRush.enable = true;
		bUseCommand = true;
	}
	...
	{
		if(CheckAttribute(pchar, "boxname.closed"))
		{
			if(CheckCharacterPerk(pchar, "Safecracker"))
			{
		        objLandInterface.Commands.HackBox.enable	= true;
	...
	if(group=="reload")
	{
		...
		if(i<n)
		{
			...
			if(i>=0)
			{
				...
				{
					if(locCur.label == "Tavern") // Rebbebion, вывод названия таверны в правом верхнем углу
			        {
						Log_SetStringToLog( LanguageConvertString(g_LocLngFileID, "Tavern")  + " '" + XI_ConvertString(""+locations[i].fastreload+"TavernName") + "'");
        			}
		        	else
			        {
