...
void CreateSeaActionsEnvironment()
{
	...
	ILogAndActions.ActiveActions.HackBox.IconNum	= 51;
	ILogAndActions.ActiveActions.HackBox.Text		= XI_ConvertString("for_quick_action_HackBox");
}
...
void BI_FastCommand()
{
	...
	if( ILogAndActions.type == "land" )
	{
		...
		switch(FComName)
		{
			...
			case "HackBox":	
				if (!LAi_IsFightMode(pchar) && CheckCharacterPerk(pchar, "Safecracker")) // в бою быстрой командой низя ломать сундук
				{
			...
			case "Reload":
	            ...
	            else Event("Control Activation","s","ChrAction");
			break;
