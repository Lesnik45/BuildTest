...
void PlaceSmugglersOnShore(string LocationId)
{
    ...
    {
        Model = "pirate_1";
		Smuggler = GetCharacter(NPC_GenerateCharacter("Rand_Smug0" + i, Model, "man", "man", 5, PIRATE, 4, true), F_SMUGGLER); // 4 дня, потом сами пропадут
...
void SetCoastalGuardPursuit()
{
	...
	for (i = 1; i <= 3; i++)
    {
        Model = "off_" + NationShortName(iNation) + "_1";
		Smuggler = GetCharacter(NPC_GenerateCharacter("Coastal_Captain0" + i, Model, "man", "man", 5, iNation, 3, true), F_MILITARY); // 3 дня, потом сами пропадут
		SetFantomParam(Smuggler);
