	...
	for (int i=0; i<COMPANION_MAX; i++)
	{
		...
		if (chrIndex != -1)
		{
			...
			if (characters[chrIndex].ship.type != SHIP_NOTUSED)
			{
				...
				iFoodQuantity   = iFoodQuantity   + makeint(GetCargoGoods(&characters[chrIndex], GOOD_FOOD) * isEquippedTalismanUse(&characters[chrIndex], "totem_31", 1.0, 0.66));
...
int CalculateShipFood(ref _chr)
{
	...
	foodNeeded = makefloat(iCrewQuantity/FOOD_BY_CREW + iPassQuantity/FOOD_BY_PASSENGERS + iSlavesQuantity/FOOD_BY_SLAVES) * isEquippedTalismanUse(_chr, "totem_31", 1.0, 0.66);
...
int CalculateShipRum(ref _character)
{
	...
	int eatRate = makeint((crewQuantity + 5.1) / RUM_BY_CREW) * isEquippedTalismanUse(_character, "totem_19", 1.0, 0.5); // Сколько жрут за день
	...
	for (int i=0; i<COMPANION_MAX; i++)
	{
		...
		if (chrIndex != -1)
		{
			...
			if (characters[chrIndex].ship.type != SHIP_NOTUSED)
			{
				...
				iRumCount     += makeint(GetCargoGoods(&characters[chrIndex], GOOD_RUM) * isEquippedTalismanUse(&characters[chrIndex], "totem_19", 1.0, 0.5));							
			}
		}
	}
...
{
	if(IsCharacterEquippedTalisman(chref, "totem_25")) return; // проверка крысобога
	if(CheckAttribute(chref, "questTemp.arsenic")) return; // проверка мышьяка
    ...
    if (iQuantity > 60 && rand(4) != 2) // шанс не жрать, если весь спектр
    {
        ...
        Log_SetStringToLog(RandSwear() + " Крысы на корабле " + chref.Ship.Name + LinkRandPhrase(" испортили ", " повредили ", " уничтожили ") + iQuantity + " шт. " + LanguageConvertString(iSeaGoods, "seg_" + Goods[iGoods].Name));
	...
	if (CheckAttribute(pchar, "CrewPayment")) // Долг
	{
		...
		if (rand(cn) > 1000)
		{
			...
			{   // любой пассажир у кого есть пристрастие может свалить
				...
				{
		            chref = &Characters[cn];
					if (CheckAttribute(chref, "loyality") && !CheckAttribute(chref, "OfficerWantToGo.DontGo") && rand(morale) == 2 && !IsCharacterEquippedTalisman(chref, "totem_25"))
					{
...
void DailyEatCrewUpdateForShip(ref rChar, bool IsCompanionTraveler) // IsCompanionTraveler - спец флаг для компаньонов-путешественников
{
	...
	if(rand(4) == 2)
	{
		...
		if(cn > 0)
		{
			...
			else
			{
				...
				cn = cn * isEquippedTalismanUse(rChar, "totem_31", 1.0, 0.66);
	...
	iCrewQty = makeint(((iCrewQty+5.1) / RUM_BY_CREW) * isEquippedTalismanUse(rChar, "totem_19", 1.0, 0.5)); // eat ratio
	//if (rChar == 0) rChar = 1;
	if(iCrewQty > 0)
	{
	...
	{
		int iMorale = -(1+rand(3));
		
		if(IsCharacterEquippedTalisman(rChar, "totem_19")) iMorale = iMorale/2;
		
		AddCrewMorale(rChar, iMorale);
	} 
	// рассчет перегруза команды на мораль <--
	...
	if(iCrewQty > 0 && CheckAttribute(PChar, "CrewPayment"))
	{
		...
		if(rand(cn) > 1000)
		{
			...
			cn = 5;  // перк у ГГ
			if(i > 0 && rand(cn) == 2 && !CheckAttribute(rChar, "OfficerWantToGo.DontGo") && !IsCharacterEquippedTalisman(rChar, "totem_25"))
			{
	...
	iCrewQty = makeint((iCrewQty+5.1) / FOOD_BY_CREW + GetPassengersQuantity(rChar) / FOOD_BY_PASSENGERS) * isEquippedTalismanUse(rChar, "totem_31", 1.0, 0.66); // eat ratio
	...
	{
		if(GetShipRemovable(rChar) && !CheckAttribute(rChar, "OfficerWantToGo.DontGo") && !IsCompanionTraveler && !IsCharacterEquippedTalisman(rChar, "totem_25")) // ПГГ, квестовые оффы и компаньоны-путешественники не бунтуют
		{
