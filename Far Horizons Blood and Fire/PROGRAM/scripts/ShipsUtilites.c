...
float FindShipSpeed(aref refCharacter)
{
	...
	fMaxSpeedZ = fMaxSpeedZ * AIShip_isPerksUse(CheckCharacterPerk(refCharacter, "MapMaker"), 1.0, 1.05) * isEquippedTalismanUse(refCharacter, "totem_23", 1.0, 1.10);
...
float FindShipTurnRate(aref refCharacter)
{
	...
	float fTurn = fTRFromWeight * fTRFromSkill * fTRFromPeople * fTRFromSailDamage * fSpeedFromHp * isEquippedTalismanUse(refCharacter, "totem_23", 1.0, 1.10);
...
void SetShipyardStore(ref NPChar)
{
	...
	if (iTest_ship == 3) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_YACHT, NPChar), "ship3");
