...
void CreateColonyCommanders()
{
	...
	for (i=0; i<MAX_COLONIES; i++)
	{
		...
		{   // мэр есть
			...
			characters[iChar].fraction = F_CITY;
        }
		
		iChar = GetCharacterIndex(colonies[i].id + "_Priest");
		if(iChar != -1) characters[iChar].fraction = F_CITY;
		
		iChar = GetCharacterIndex(colonies[i].id + "_waitress");
		if(iChar != -1) characters[iChar].fraction = F_CITY;
		
		iChar = GetCharacterIndex(colonies[i].id + "_tavernkeeper");
		if(iChar != -1) characters[iChar].fraction = F_CITY;
		
		iChar = GetCharacterIndex(colonies[i].id + "_shipyarder");
		if(iChar != -1) characters[iChar].fraction = F_CITY;
		
		iChar = GetCharacterIndex(colonies[i].id + "_PortMan");
		if(iChar != -1) characters[iChar].fraction = F_CITY;
		
		iChar = GetCharacterIndex(colonies[i].id + "_Smuggler");
		if(iChar != -1) characters[iChar].fraction = F_SMUGGLER;
		
		iChar = GetCharacterIndex(colonies[i].id + "_Hostess");
		if(iChar != -1) characters[iChar].fraction = F_CITY;
		...
		characters[iChar].fraction    = F_MILITARY; //
		...
		{
			characters[iChar].Fort.Cannons.Type.2 = CANNON_TYPE_CANNON_LBS32;
		}
...
bool PrepareTownBattle()
{
    ...
    how     = sti(nulChr.GenQuestFort.(sCity).howsoldier);
...
void SetTownMayor(ref Ch, int natGover)
{
	...
	else
	{
	    switch(rand(11))
        {
            case 0: rmodel = "banker_1";   break;
            case 1: rmodel = "banker_1";   break;
            case 2: rmodel = "banker_1";   break;
            case 3: rmodel = "banker_1";   break;
            case 4: rmodel = "banker_1";   break;
            case 5: rmodel = "banker_1";   break;
            case 6: rmodel = "trader_1";   break;
            case 7: rmodel = "trader_1";   break;
            case 8: rmodel = "trader_1";   break;
            case 9: rmodel = "trader_1";   break;
            case 10: rmodel = "trader_1";   break;
            case 11: rmodel = "shipbuilder_1";   break;
        }
	}
