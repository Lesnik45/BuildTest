...
void InitPsHeros()
{
	...
	for (n=1; n<=heroQty; n++)
	{
        ...
        {
			PsHeroQty++;
			ch = GetCharacter(NPC_GenerateCharacter("PsHero_" + PsHeroQty, "off_hol_2", "man", "man", 5, PIRATE, -1, true), F_OFFICER);
...
void PGG_CheckDead(ref chr)
{
	...
	if (IsCompanion(chr) && GetRemovable(chr)) return;
...
string PGG_FindRandomTownByNation(int _nation)
{
	...
	{
		if (sti(Colonies[i].nation) == _nation && Colonies[i].from_sea != "") //в сухопутных не будут. //18.06.07 fix ГПК
		{
...
//		LAi_warrior_SetStay(chr, true); //можно убрать, будет шататься по бухте.
		...
		{
			sTemp = "pirate_1";
			sld = GetCharacter(NPC_GenerateCharacter("Rand_Enemy0" + i, sTemp, "man", "man", 5, PIRATE, 3, true), F_SMUGGLER);
...
void PGG_Q1PlaceShipsNearIsland()
{
	...
	{
		sTmp = "pirate_1";
		chr = GetCharacter(NPC_GenerateCharacter("RandQuestCap_0" + i, sTmp, "man", "man", MakeInt(iRank/2 + rand(iRank)), iNation, iLifeDay, true), F_MILITARY);
...
void PGG_Q1LocationLoaded(string qName)
{
	...
	{
		attrName = "pirate_"+ i;
		chr = GetCharacter(NPC_GenerateCharacter(attrName, "pirate_" + (rand(15) + 1), "man", "man", 15, PIRATE, 0, true), F_PIRATE);
	...
	if (!bQuestCheckProcessFreeze)
	{
		if (IsEntity(worldMap))
		{
			...
			for (i=1; i<5; i++)
			{
				...
				chr = GetCharacter(NPC_GenerateCharacter(attrName, "pirate_" + (rand(15) + 1), "man", "man", 15, PIRATE, 1, true), F_PIRATE);
				SetFantomParam(chr);
				chr.dialog.Filename = "Quest\sailors_dialog.c";
				LAi_SetWarriorType(chr);
				LAi_SetLoginTime(chr, 0.0, 24.0);
