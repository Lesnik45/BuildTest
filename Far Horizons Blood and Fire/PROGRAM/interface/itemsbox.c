...
void InterfaceInitButtons(ref _refCharacter)
{
	...
	switch(sInterfaceType)
	{
		...
		case INTERFACETYPE_DEADMAN:
			...
			if (IsCharacterEquippedTalisman(pchar, "totem_32") && IsAztecSkullOfficer(arDeadChar) && LAi_IsDead(characters[sti(arDeadChar.index)]) && CheckNPCQuestDate(pchar, "totem_32"))
			{
...
void IDoExit(int exitCode)
{
	...
	if(CheckAttribute(pchar, "InLockpickedChest"))  DeleteAttribute(pchar, "InLockpickedChest");
}
...
void ProcCommand()
{
	...
	switch(nodName)
	{
		...
		case "SKULL_BUTTON":
			...
			if(comName=="activate" || comName=="click")
			{
				SetNPCQuestDate(refCharacter, "totem_32"); //запомним день
