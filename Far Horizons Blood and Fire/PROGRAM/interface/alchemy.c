...
{
	int 	i, iNum;
	string 	attributeName, sAttr, sItemId, sItmId, perkCond;
	ref     itm; 
	aref 	arImgGrp, rType;
	aref rootItems;
	...
	GameInterface.ITEMS_SCROLL.ImagesGroup.t0 = "interfaces\empty_item.tga";
	...
	for(int n = 0; n < ITEMS_QUANTITY; n++)
	{
		...
		if(CheckAttribute(itm, "id") && CheckAttribute(itm, "multiobject") && isMultiObjectKnown(itm.id))
		{
			// проверка на необходимы перки -->
			if (CheckAttribute(itm, "CraftCondition"))
			{
 		        makearef(rootItems, itm.CraftCondition);
				for (i = 0; i < GetAttributesNum(rootItems); i++)
 		    	{
 		   	        perkCond = GetAttributeName(GetAttributeN(rootItems, i));
					
 		   	        if (!CheckCharacterPerk(pchar, perkCond)) isShowItem = false;
 		     	}
			}
			else isShowItem = true;
			
			if(!isShowItem) continue; //Нету нужных перков для крафта - в сад!
			...
			//if(CheckAlchemy(sCurItem) < 1) continue; //Для будущей галочки "Все, что можно скрафтить"
			m++;
		}
	}
}
...
void onGetAllBtnClick()
{
	...
	if(CheckAttribute(rItem, "rare"))
	{
	    if(rItem.rare == "Common") iChance = 75;
	    if(rItem.rare == "rare") iChance = 50;
    	if(rItem.rare == "Epic") iChance = 25;
    	if(rItem.rare == "Legendary") iChance = 1;
	}
	
	if(CheckCharacterPerk(refCharacter, "WastefulCreator") && rItem.groupID != AMMO_ITEM_TYPE && (rand(99)+ 1) <= iChance)
	{
		Log_TestInfo("Получена копия предмета "+ sCurItem +"!");
		AddItems(refCharacter, sCurItem, 1);
	}
