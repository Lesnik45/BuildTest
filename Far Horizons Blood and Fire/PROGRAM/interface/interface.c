...
{
	if(procInterfacePrepare(INTERFACE_LOCKPICKING))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_LOCKPICKING;
		InitInterface_GM(Interfaces[CurrentInterface].IniFile);
	}
}

void LaunchTest(ref refCharacter)
{
	if(procInterfacePrepare(INTERFACE_TEST))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_TEST;
...
void LaunchBetaTeleportScreen()
{
	if(procInterfacePrepare(INTERFACE_BETA_TELEPORT))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_BETA_TELEPORT;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}
