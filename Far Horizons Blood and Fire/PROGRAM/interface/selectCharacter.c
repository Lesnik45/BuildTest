...
void InitInterface(string iniName)
{
	...
	SendMessage(&GameInterface,"lsll",MSG_INTERFACE_MSG_TO_NODE,"MainHero", 5, 1);
    ...
    {   // иначе уже загружен и выбран гг, смотрим настройки и идем обратно
		SetVariable(false); // Было true
	}
	else
	{
	    SetVariable(true);  // Было false
	}
...
void DisableEnable_CheckProcess() // Rebbebion, калибровка параметров для уровней сложности
{
	if (MOD_SKILL_ENEMY_RATE == 10)
	{		
		if(!SendMessage(&GameInterface,"lsll",MSG_INTERFACE_MSG_TO_NODE, "CHECK_HARDCORE", 3, 1)) // если хардкорный режим включён
		{
			iBackHardcore = 0; // сохраняем предыдущее значение
			SendMessage(&GameInterface,"lslll",MSG_INTERFACE_MSG_TO_NODE,"CHECK_HARDCORE", 2, 1, 1 ); // включаем
		}
		else
		{
			iBackHardcore = 1; // сохраняем предыдущее значение
		}
	
		SetClickable("CHECK_HARDCORE", false);
		
		Button_SetEnable("CHECK_HARDCORE", false);		
	}
	else
	{		
		if(iBackHardcore > -1 )
		{
			SendMessage(&GameInterface,"lslll",MSG_INTERFACE_MSG_TO_NODE,"CHECK_HARDCORE", 2, 1, iBackHardcore);
			iBackHardcore = -1;
		}
		
		SetClickable("CHECK_HARDCORE", true);

		Button_SetEnable("CHECK_HARDCORE", true);
	}
}
