...
void SkipSailorPayment()
{
	...
	for(int i=0; i<COMPANION_MAX; i++)
	{
		...
		if(cn>=0)
		{
			...
			if (i > 0 && CheckAttribute(chref, "loyality") && !CheckAttribute(chref, "OfficerWantToGo.DontGo") && !IsCharacterEquippedTalisman(chref, "totem_25"))
			{
	...
	{   // любой пассажир у кого есть пристрастие может свалить если наши дела ему не по душе
		...
		if (iPassenger != -1)
		{
			...
			if (CheckAttribute(chref, "loyality") && !CheckAttribute(chref, "OfficerWantToGo.DontGo") && !IsCharacterEquippedTalisman(chref, "totem_25"))
			{
    			chref.loyality = makeint(chref.loyality) - 1;
			}
		}
