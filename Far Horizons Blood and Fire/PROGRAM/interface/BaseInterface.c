...
{
	// Rebbebion, переделка файлов СЖ, теперь один файл будет отвечать за определённый тип записей. То есть:
	questTextFileName[0] = "PROGRAM\QuestBook\QuestBook_Story.txt"; // квесты основного сюжета
	questTextFileName[1] = "PROGRAM\QuestBook\QuestBook_SideQuests.txt"; // побочные квесты
	questTextFileName[2] = "PROGRAM\QuestBook\QuestBook_GenQuests.txt"; // генераторные квесты
	questTextFileName[3] = "PROGRAM\QuestBook\QuestBook_MythsAndLegends.txt"; // мифы и легенды
    questTextFileName[4] = "PROGRAM\QuestBook\QuestBook_Documents.txt"; // документы
...
void InitInterfaceTables()
{
	...
	//Телепорт по островам
	Interfaces[INTERFACE_BETA_TELEPORT].SectionName = "interface\beta_teleport.c";
	Interfaces[INTERFACE_BETA_TELEPORT].IniFile = "RESOURCE\INI\INTERFACES\beta_teleport.ini";
	
	// интерфейс взлома
	Interfaces[INTERFACE_LOCKPICKING].SectionName = "interface\lockpicking.c";
	Interfaces[INTERFACE_LOCKPICKING].IniFile = "RESOURCE\INI\INTERFACES\lockpicking.ini";	
	
	// тестовый интерфейс
	Interfaces[INTERFACE_TEST].SectionName = "interface\test.c";
	Interfaces[INTERFACE_TEST].IniFile = "RESOURCE\INI\INTERFACES\test.ini";	
}