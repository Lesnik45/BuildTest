...
void AddToTable(int _mode)
{
	...
	for (i = ITEMS_QUANTITY; i < TOTAL_ITEMS; i++)
	{
		...
		if(HasSubStr(rItem.id, "EmptyItem_")) continue;
		...
		if (leftQty > 0 || rightQty > 0)
		{
			...
			if(findsubstr(rItem.id, "recipe_" , 0) != -1) GameInterface.TABLE_LIST.(row).td4.str = LanguageConvertString(idLngFile, rItem.name) + " (" + LanguageConvertString(idLngFile, "Itmname_"+rItem.result) + ")";
			else GameInterface.TABLE_LIST.(row).td4.str = LanguageConvertString(idLngFile, rItem.name);
...
void ShowGoodsInfo(int iGoodIndex)
{
	...
	if(findsubstr(Items[iGoodIndex].id, "recipe_" , 0) != -1) sHeader = LanguageConvertString(lngFileID, Items[iGoodIndex].name) + " (" + LanguageConvertString(lngFileID, "Itmname_"+Items[iGoodIndex].result) + ")";
	else sHeader = LanguageConvertString(lngFileID, Items[iGoodIndex].name);
