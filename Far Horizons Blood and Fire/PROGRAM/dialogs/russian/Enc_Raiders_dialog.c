void ProcessDialogEvent()
{
	...
	if(CheckCharacterPerk(pchar, "Bribe")) iRand = 60;
	if(CheckCharacterPerk(pchar, "AGOS")) iRand = 20;
	
	
	if(CheckCharacterPerk(pchar, "FearsomeLook")) iFear = 70;
	if(CheckCharacterPerk(pchar, "AGOS")) iFear = 55;
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Node_3":
			if(rand(100) <= iFear || CheckCharacterTalent(pchar, "Seducer"))
			{
