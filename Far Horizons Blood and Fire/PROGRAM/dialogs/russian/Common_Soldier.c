...
void ProcessDialogEvent()
{
    ...
    // вызов диалога по городам <--
    int iTest, iTemp, iRand, iBribeSumm, iFear;
	string sTemp, sCity;
	...
	if(CheckCharacterPerk(pchar, "Bribe")) iRand = 40;
	if(CheckCharacterPerk(pchar, "AGOS")) iRand = 80;
	
	
	if(CheckCharacterPerk(pchar, "FearsomeLook")) iFear = 30;
	if(CheckCharacterPerk(pchar, "AGOS")) iFear = 45;
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "fight":
			...
			//Преследование ГГ стражниками на выходе из города
			sCity = NPChar.City + "_ExitTown";
			sTemp = "Hero_ExitTown_SoldierChase";
			
			PChar.quest.(sTemp).win_condition.l1 = "location";
	        PChar.quest.(sTemp).win_condition.l1.location = sCity;
	        PChar.quest.(sTemp).function = "Hero_ExitTown_SoldierChasePrepare";
	        PChar.Hero_ExitTown_SoldierChase.nation = sti(NPChar.nation);
	        PChar.Hero_ExitTown_SoldierChase.city = NPChar.city;
		...
		case "First time":			
			...
			if (GetNationRelation2MainCharacter(sti(NPChar.nation)) == RELATION_ENEMY && sti(NPChar.nation) != PIRATE)
			{
				...
				link.l2 = "Какая неудача, так глупо попасться... Сдаюсь! Вот мое оружие..."
				link.l2.go = "Spalilsya";
				
				if(!isDay())
				{
					link.l3 = "(Подкуп)";
					if(rand(100) <= iRand && sti(pchar.money) >= iBribeSumm) link.l3.go = "Bribe";
					else link.l3.go = "PegYou";
				
					link.l4 = "(Устрашение: Получите +2 к силе карты XD)";
					if(rand(100) <= iFear) link.l4.go = "Fear";
					else link.l4.go = "PegYou";
				}
			}
			else
			{
					...
					else
					{
						...
						if(isDay())
						{
							if (findsubstr(pchar.location.from_sea, "_town" , 0) != -1) //если причалил в городе
							{
								...
								if (GetSummonSkillFromName(pchar, SKILL_SNEAK) < (10+rand(50)+rand(50)) * IsCharacterEquippedTalisman(pchar, "totem_10", 1, 0.85))
					    		{
					    			link.l1.go = "PegYou";
					    		}
					    		else
						...
						{
							link.l1 = "Я не шпиён! Мамой клянусь!";
							if (GetSummonSkillFromName(pchar, SKILL_SNEAK) < (10+rand(50)+rand(50)) * IsCharacterEquippedTalisman(pchar, "totem_10", 1, 0.85))
					    	{
					    		link.l1.go = "PegYou";
					    	}
							...
							link.l2 = "(Подкуп)";
							if(rand(100) <= iRand && sti(pchar.money) >= iBribeSumm) link.l2.go = "Bribe";
							else link.l2.go = "PegYou";
						
							link.l3 = "(Устрашение: Получите +2 к силе карты XD)";
							if(rand(100) <= iFear) link.l3.go = "Fear";
							else link.l3.go = "PegYou";
						}
					}
				}
		...
		case "PegYou":
			dialog.text = RandPhraseSimple("Сдаётся мне, что это обман... Давай-ка пройдем в комендатуру, "+ GetSexPhrase("голубчик","голубушка") +", там разберёмся...", "Хм, что-то подсказывает мне, что ты не "+ GetSexPhrase("тот","та") +", за кого себя выдаешь... Немедленно сдайте оружие, " + GetAddress_Form(npchar) + ", и следуйте за мной для дальнейшего разбирательства!");
			link.l1 = RandPhraseSimple("Как бы не так!", "После дождичка, в четверг...");
			link.l1.go = "fight";
			
			link.l2 = "Какая неудача, так глупо попасться... Сдаюсь! Вот моё оружие..."
			link.l2.go = "Spalilsya";
			if (sti(pchar.questTemp.stels.landsoldier) != GetDataDay())
			{
				AddCharacterExpToSkill(pchar, SKILL_SNEAK, 40);
				pchar.questTemp.stels.landsoldier = GetDataDay();
			}
		break;
		case "NotPegYou":
			dialog.text = RandPhraseSimple("А-а-а, вижу... Всё в порядке, вы можете идти, " + GetAddress_Form(pchar) + ".", "Что-то я немного подустал в карауле... Все в порядке, " + GetAddress_Form(pchar) + ", прошу прощения.");
			link.l1 = "Так-то!";
			link.l1.go = "exit";
			if (sti(pchar.questTemp.stels.landsoldier) != GetDataDay())
			{
		...
		case "Bribe":
		    AddMoneyToCharacter(pchar, -iBribeSumm)
			NextDiag.CurrentNode = NextDiag.TempNode;
			DialogExit();
		break;
		
		case "Fear":
		    if(CheckAttribute(pchar, "questTemp.Fear."+NPChar.city))
			{
		        sCity = NPChar.city;
                pchar.questTemp.Fear.(sCity) = true;
    			SetCityFearTimerFunction(NPChar.city + "_RemoveCityFearAttribute", "RemoveCityFearAttribute", 0, 0, 1, sCity);
			}
			NextDiag.CurrentNode = NextDiag.TempNode;
			DialogExit();
		break;
		...
		case "Spalilsya":
			...
			NextDiag.CurrentNode = NextDiag.TempNode;
			DialogExit();
			DoQuestReloadToLocation(sCity, "Reload", "Reload3", "");
		break;
	}
}