...
void QuestToSeaLogin_Launch()
{
	...
	SendMessage(&fader, "ls",FADER_PICTURE0,"loading\sea_0" + rand(2) + ".tga");
...
//Экку Korsar - Костыль, но я чуть не сломал свою бошку, пока пытался вычислить, где почасово меняется время сразу и везде
void SetPoisonTimerFunctionParam(string _Qname, string _name int _year, int _month, int _day, int _hour, ref _chr, string _bladeid)
{
    while (_hour > 23)
	{
		_hour -= 23;
	...
	PChar.quest.(_Qname).win_condition.l1            = "Timer";
    PChar.quest.(_Qname).win_condition.l1.date.day   = GetAddingDataDay(_year, _month, _day);
    PChar.quest.(_Qname).win_condition.l1.date.month = GetAddingDataMonth(_year, _month, _day);
    PChar.quest.(_Qname).win_condition.l1.date.year  = GetAddingDataYear(_year, _month, _day);
    PChar.quest.(_Qname).win_condition.l1.date.hour  = _hour;
    PChar.quest.(_Qname).function                    = _name;
    PChar.quest.(_Qname).character                   = _chr.id;
    PChar.quest.(_Qname).blade                       = _bladeid;
}

//Экку Korsar - Костыль, но я чуть не сломал свою бошку, пока пытался вычислить, где почасово меняется время сразу и везде
void SetRatPoisonTimerFunctionParam(string _Qname, string _name int _year, int _month, int _day, ref _chr)
{
	PChar.quest.(_Qname).win_condition.l1            = "Timer";
    PChar.quest.(_Qname).win_condition.l1.date.day   = GetAddingDataDay(_year, _month, _day);
    PChar.quest.(_Qname).win_condition.l1.date.month = GetAddingDataMonth(_year, _month, _day);
...
//Экку Korsar - Костыль, но я чуть не сломал свою бошку, пока пытался вычислить, где почасово меняется время сразу и везде
void SetPotionTimerFunctionParam(string _Qname, string _name int _year, int _month, int _day, int _hour, ref _chr, string _effect)
{
    while (_hour > 23)
	{
		_hour -= 23;
    ...
    PChar.quest.(_Qname).function                    = _name;
    PChar.quest.(_Qname).character                   = _chr.id;
    PChar.quest.(_Qname).effect                      = _effect;
}

// Таймер на запуК стражи :)
void SetCityFearTimerFunction(string _Qname, string _name, int _year, int _month, int _day, string _City)
{
    PChar.quest.(_Qname).win_condition.l1            = "Timer";
    PChar.quest.(_Qname).win_condition.l1.date.day   = GetAddingDataDay(_year, _month, _day);
    PChar.quest.(_Qname).win_condition.l1.date.month = GetAddingDataMonth(_year, _month, _day);
    PChar.quest.(_Qname).win_condition.l1.date.year  = GetAddingDataYear(_year, _month, _day);
    PChar.quest.(_Qname).function					 = _name;
    PChar.quest.(_Qname).city					     = _City;
}

//Экку Korsar - отложеный вызов квестовой функции с доп приколами
void DoPotionQuestFunctionDelay(string questFunctionName, string questFunction, ref questchr, float delayInSecond)
{
	pchar.PostEventQuest.questDelay.(questFunctionName).function = questFunction;
	pchar.PostEventQuest.questDelay.(questFunctionName).character = questchr.id;
	DoQuestCheckDelay(questFunctionName, delayInSecond);
}
